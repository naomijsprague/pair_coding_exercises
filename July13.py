# -*- coding: utf-8 -*-
"""
Created on Mon Jul 13 09:20:24 2015
...
def find_words(words): 
    more_words = words.replace('\n',' ')
    word_soup = more_words.split(' ')
    new_list = []
    ...
    for word in new_list:
        another_list.append(''.join(letter for letter in word if letter.isalnum()))
    return another_list        
        
        
def make_lists(list1, list2):
    one_list = find_words(list1)   
    two_list = find_words(list2)
    intersect = [val for val in one_list if val in two_list]
    return intersect
